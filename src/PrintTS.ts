...
function printUnionTypeWithHeader(
    printer: Printer,
    unionType: flow.UnionType,
...
function printObjectTypeWithoutMixins(
    printer: Printer,
...
function printType(
    ...
    switch (flowType.kind) {
        ...
        default:
            throw new Error(
                `Unrecognized Flow type: ${(<flow.Type>flowType).kind}`
            );
    }
}
...
function printExpression(
    ...
    switch (expr.kind) {
        ...
        default:
            throw new Error(
                `Unrecognized Flow expression: ${(<flow.Expression>expr).kind}`
            );
    }
}
...
function printStatement(
    ...
    stat: flow.Statement,
    forceExport: boolean,
    typeConfig: PrintTypeConfig
): void {
    switch (stat.kind) {
        case "UseStrictStat": {
            printer.write(`'use strict';`);
            break;
        }
        case "TypeAliasDecl": {
        ...
        case "InterfaceDecl": {
            ...
            if (stat.baseTypes.length > 0) {
                ...
                printTypeArray(
                    printer,
                    stat.baseTypes.concat(stat.interfaceType.mixinTypes),
                    ", ",
                    typeConfig
                );
            }
        ...
        case "ImportNameStat": {
            printer.write(
                `import {${stat.names.join(", ")}} from ${stat.source};`
            );
            break;
        }
        case "ExportImportNameStat": {
            if (Array.isArray(stat.name)) {
                printer.write(
                    `export {${stat.name.join(", ")}} from ${stat.source};`
                );
            } else {
                printer.write(`export {${stat.name}} from ${stat.source};`);
            }
            break;
        }
        ...
        case "ModuleExportEqualStat": {
            printer.write("export default");
            printer.write("{");
            printer.writeLn();
            printer.pushIndent();
            for (let i = 0; i < stat.properties.length; i++) {
                const prop = stat.properties[i];
                printer.writeIndent();
                printer.write(prop.key);
                if (prop.value) {
                    printer.write(": ");
        ...
        default:
            throw new Error(
                `Unrecognized Flow statement: ${(<flow.Statement>stat).kind}`
            );
    }
}

export function printTypeScript(
    program: flow.FlowProgram,
    forceExport: boolean,
    typeConfig: PrintTypeConfig
): string {
